...
def settlement_export_csv(user: CurrentUser = Depends(require_role("owner", "accountant")), year: int | None = None, month: int | None = None):
    ...
    w.writerow(["پزشک فعال", b["doctor_name"]])
    w.writerow([])
    w.writerow(["ردیف", "مبلغ (ریال)"])
    w.writerow(["درآمد پزشک", b["doctor_revenue"]])
    w.writerow(["درآمد شریک (اپراتور خودِ شریک)", b["partner_self_revenue"]])
    w.writerow(["هزینه‌های مصرفی (مربوط به کار پزشک)", b["consumable_expenses"]])
    w.writerow(["هزینه‌های غیرمصرفی", b["non_consumable_expenses"]])
    w.writerow(["سهم پزشک", b["doctor_share"]])
    w.writerow(["باقیمانده قابل تقسیم بین کلینیک و شریک", b["remaining"]])
