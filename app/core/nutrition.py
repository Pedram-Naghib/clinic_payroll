...
def compute_shares(
    ...
    doctor_pct: int = 50, clinic_pct: int = 50, partner_pct: int = 50, is_partner_self: bool = False,
) -> dict:
    ...
    is_partner_self: the revenue was generated by the partner personally
    (she's the operator on those rows), not a separate treating doctor --
    there's no third-party cut to carve out, so the doctor-share step is
    skipped and the full remaining (gross minus all expenses) splits
    straight between clinic and partner."""
    if is_partner_self:
        doctor_consumable_cut = 0
        other_consumable_cut = consumable_expenses
        doctor_share = 0
        remaining = gross_revenue - consumable_expenses - non_consumable_expenses
    else:
    ...
    return {
        ...
        "is_partner_self": is_partner_self,
    }
...
def compute_settlement_for_period(conn, period_start: str, period_end: str, doctor_id: int) -> dict:
    ...
    breakdown = compute_shares(
        ...
        is_partner_self=bool(doctor["is_partner_self"]),
    )
