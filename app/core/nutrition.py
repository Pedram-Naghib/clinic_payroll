...
def add_doctor(conn, d: DoctorInput, contract: ContractInput) -> int:
    cur = conn.execute(
        """INSERT INTO nutrition_doctors (full_name, phone, contract_start, contract_end, notes, is_partner_self, active)
           VALUES (?, ?, ?, ?, ?, ?, 0)""",
        (d.full_name, d.phone, d.contract_start, d.contract_end, d.notes, int(d.is_partner_self)),
    )
...
def update_doctor(conn, doctor_id: int, d: DoctorInput) -> None:
    conn.execute(
        """UPDATE nutrition_doctors SET full_name = ?, phone = ?, contract_start = ?,
               contract_end = ?, notes = ?, is_partner_self = ? WHERE id = ?""",
        (d.full_name, d.phone, d.contract_start, d.contract_end, d.notes, int(d.is_partner_self), doctor_id),
    )
    conn.commit()
...
def get_partner_doctor(conn):
    """The operator row that represents the partner performing services
    herself (is_partner_self=1), if one exists -- used for the partner
    invoice's "handed to" name."""
    return conn.execute("SELECT * FROM nutrition_doctors WHERE is_partner_self = 1 LIMIT 1").fetchone()
...
def compute_shares(
    doctor_revenue: int, partner_self_revenue: int, consumable_expenses: int, non_consumable_expenses: int,
    ...
    return {
        "gross_revenue": doctor_revenue + partner_self_revenue,
...
def compute_settlement_for_period(conn, period_start: str, period_end: str, doctor_id: int) -> dict:
    """doctor_id is the *active* operator (governs which contract's
    percentages apply) -- but revenue is pulled from BOTH streams
    regardless of which one is "active": whatever's tagged to this active
    operator (if she isn't herself the partner) plus everything tagged to
    whichever operator(s) are flagged is_partner_self. See compute_shares."""
    doctor = get_doctor(conn, doctor_id)
    contract = get_contract_for_doctor(conn, doctor_id)
    if doctor["is_partner_self"]:
        doctor_revenue = 0
    else:
        doctor_revenue = conn.execute(
    ...
    partner_self_revenue = conn.execute(
        """SELECT COALESCE(SUM(r.amount), 0) AS s FROM nutrition_revenues r
           JOIN nutrition_doctors d ON d.id = r.doctor_id
           WHERE d.is_partner_self = 1 AND r.date >= ? AND r.date < ?""",
    ...
    breakdown = compute_shares(
        doctor_revenue, partner_self_revenue, consumable, non_consumable,
...
def save_settlement(
    conn, period_start: str, period_end: str, breakdown: dict, generated_by: int | None,
    adjustment_amount: int = 0, adjustment_note: str | None = None,
) -> int:
    """adjustment_amount/adjustment_note record a manual, signed correction on
    top of the formula's partner_share -- for months where the normal
    cash-flow assumption (clinic fronts expenses out of shared revenue)
    doesn't hold, e.g. the partner personally paid every expense. The
    formula's shares themselves are never edited; this is a separate,
    ...
    if existing is not None:
        conn.execute(
            ...
            (
                ...
                breakdown["partner_share"], adjustment_amount, adjustment_note, generated_by, existing["id"],
            ),
        )
        conn.commit()
        return existing["id"]
    cur = conn.execute(
        """INSERT INTO nutrition_settlements (period_start, period_end, doctor_id, gross_revenue,
               consumable_expenses, non_consumable_expenses, doctor_share, clinic_share, partner_share,
               adjustment_amount, adjustment_note, generated_by)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (
            ...
            breakdown["doctor_share"], breakdown["clinic_share"], breakdown["partner_share"],
            adjustment_amount, adjustment_note, generated_by,
        ),
    )
    conn.commit()
